#define MTWITHTASK    // Multitasking mit Task oder ThreadPool
...
namespace BuildDEMFile {
   ...
   class FileBuilder {
      ...
      public bool Create(List<string> DemDataPaths,
         ...
         if (tiles4zoomlevel.Count > 0) {
            ...
            FSoftUtils.TaskQueue tq = maxthreads != 1 ?
                                          ...
                                          null;
#else
            CalculationThreadPoolExt calctp = maxthreads != 1 ?
                                                   new CalculationThreadPoolExt(ThreadMsg, maxthreads) :
                                                   null;
#endif
      ...
      class CalculationThreadPoolExt : ThreadPoolExt {
         ...
         public CalculationThreadPoolExt(WaitCallback msgfunc, int maxthreads) : base(msgfunc, null, maxthreads) { }

         protected override void DoWork(object para) {

            //Console.WriteLine("Threads: {0}", ActualThreads);
