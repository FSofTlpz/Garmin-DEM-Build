   ...
   class TileEncoderProd {
      ...
      public class BitEncoding {
         ...
         public void EncodeHybrid(int data, int hunit) {
            ...
            int maxm = valrange.GetMaxLengthZeroBits();
            if (m <= maxm) {
         ...
         public void EncodeBigBin(int data, EncodeMode mode) {
            ...
            int length0 = valrange.GetMaxLengthZeroBits() + 1; // 1 Bit mehr als Max. als BigBin-Kennung
            ...
            if (mode == EncodeMode.BigBinaryL1 ||
               ...
               valrange.GetValueRangeBigBin(out min, out max);
            } else {
               valrange.GetValueRangeBigBin(out min, out max);
            }
            ...
            int bitcount = valrange.GetBigBinBits();
         ...
         public void EncodeLength(int count0bits) {
         ...
         public void EncodePlateau(int length, int startcolumn, int startline, int linelength, PlateauTable plateauTable4Tile) {
            List<char> PlateauUnits = new List<char>();
      ...
      class ValueRange {
         ...
         /// </summary>
         /// <returns></returns>
         public int GetMaxLengthZeroBits() {
            if (maxheight < 2)
               return 15;
            if (maxheight < 4)
               return 16;
         ...
         public void GetValueRangeHybrid(int hunit, out int min, out int max, int iPlateauLengthBinBits) {
            int lbits = GetMaxLengthZeroBits();
         ...
         /// <param name="max">größter verwendbarer Wert</param>
         /// <param name="iPlateauLengthBinBits">Anzahl der Binbits für die ev. vorausgehende Plateaulänge</param>
         public void GetValueRangeLength0(out int min, out int max, int iPlateauLengthBinBits) {
            int lbits = GetMaxLengthZeroBits();
            if (iPlateauLengthBinBits >= 0) // bei Plateaufollower weniger Bit erlaubt
               lbits -= iPlateauLengthBinBits + 1;
            min = -(lbits / 2);
         ...
         /// <param name="max">größter verwendbarer Wert</param>
         /// <param name="iPlateauLengthBinBits">Anzahl der Binbits für die ev. vorausgehende Plateaulänge</param>
         public void GetValueRangeLength1(out int min, out int max, int iPlateauLengthBinBits) {
            int lbits = GetMaxLengthZeroBits();
            if (iPlateauLengthBinBits >= 0) // bei Plateaufollower weniger Bit erlaubt
               lbits -= iPlateauLengthBinBits + 1;
            max = lbits / 2 + 1;
         ...
         /// <param name="max">größter verwendbarer Wert</param>
         /// <param name="iPlateauLengthBinBits">Anzahl der Binbits für die ev. vorausgehende Plateaulänge</param>
         public void GetValueRangeLength2(out int min, out int max, int iPlateauLengthBinBits) {
            int lbits = GetMaxLengthZeroBits();
            if (iPlateauLengthBinBits >= 0) // bei Plateaufollower weniger Bit erlaubt
               lbits -= iPlateauLengthBinBits + 1;
            max = lbits / 2;
         ...
         public void GetValueRangeBigBin(out int min, out int max) {
         ...
         /// <param name="min">kleinster verwendbarer Wert</param>
         /// <param name="max">größter verwendbarer Wert</param>
         public void GetValueRangeBigBinL1(out int min, out int max) {
         ...
         public int Wrap(int data, out bool wrapped, ref EncodeMode em, int hunit, int iPlateauLengthBinBits) {
            ...
            switch (em) {
               case EncodeMode.Length0:
                  ...
                  Valuerange.GetValueRangeLength0(out minval, out maxval, iPlateauLengthBinBits);
               ...
               case EncodeMode.Length1:
                  ...
                  Valuerange.GetValueRangeLength1(out minval, out maxval, iPlateauLengthBinBits);
                  if (datawrapped != int.MinValue) {
                     if (datawrapped < minval || maxval < datawrapped) {
                        datawrapped = int.MinValue; // wird doch nicht benötigt
               ...
               case EncodeMode.Length2:
                  ...
                  Valuerange.GetValueRangeLength2(out minval, out maxval, iPlateauLengthBinBits);
                  if (datawrapped != int.MinValue) {
                     if (datawrapped < minval || maxval < datawrapped) {
                        datawrapped = int.MinValue; // wird doch nicht benötigt
               ...
               case EncodeMode.Hybrid:
                  ...
                  Valuerange.GetValueRangeHybrid(hunit, out minval, out maxval, iPlateauLengthBinBits);
                  if (datawrapped != int.MinValue) {
                     if (datawrapped < minval || maxval < datawrapped) {
                        datawrapped = int.MinValue; // wird doch nicht benötigt
                        em = EncodeMode.BigBinary;
            ...
            if (datawrapped == int.MinValue) {
               switch (em) {
                  ...
                  case EncodeMode.BigBinaryL1:
                     Valuerange.GetValueRangeBigBinL1(out minval, out maxval);
                     if (data > maxval)
                        datawrapped = WrapDown(data);
                     else if (data < minval)
                        datawrapped = WrapUp(data);
