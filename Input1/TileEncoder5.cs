...
namespace Encoder {
   ...
   public class TileEncoder {
      ...
      abstract class HeightUnit {
         ...
         virtual protected void PostSetHunit() { }
      ...
      class HeightUnit4PlateaufollowerDdiffZero : HeightUnitStd {
         ...
         protected override void SpecialDataSetting(int data) {
            if (data != 0 && ElemCount % 2 == 0)
               AbsSum--;

            if (data < 0)
               AbsSum++;

            base.SpecialDataSetting(data);
         }

         protected override void PostSetHunit() {
            if (Value == 0 &&
                AbsSum * 2 > ElemCount)
               SetValue(1);
         }
      ...
      class CodingType {
         ...
         /// <param name="ismaxheight">wenn true wird die max. Höhendiff. geliefert</param>
         public CodingType(int hunitvalue, bool ismaxheight) {
            SumH = 0;
            ElemCount = 0;
            if (hunitvalue < 0)
         ...
         /// </summary>
         /// <param name="exponent"></param>
         public CodingType(int exponent) {
            SumH = 0;
            ElemCount = 0;
            if (exponent == 0)
               SetHunitValue(0);
         ...
         virtual public void AddValue(int data) {
            SumH += Math.Abs(data);
         ...
         /// <param name="abssum">bisherige Summe</param>
         /// <param name="elemcount">Anzahl der Elemente</param>
         protected void SetHunit4SumAndElemcount(int abssum, int elemcount, int maxheigthdiff = int.MinValue) {
            int counter = abssum + 1 + GetHeightUnitTableDelta4MaxHeight(maxheigthdiff > 0 ? maxheigthdiff : this.maxheigthdiff);
            int denominator = elemcount + 1;
            if (counter >= denominator)
               SetHunitValue(counter / denominator);
            else
               SetHunitValue(0);
         }
      ...
      class LengthCodingType {
         ...
         /// <summary>
         /// akt. Summe der Bewertungen
         /// </summary>
         public int LbsSum { get; protected set; }
         /// <summary>
         /// akt. Anzahl Elemente
         /// </summary>
         public int ElemCount { get; protected set; }
         ...
         /// <param name="data"></param>
         /// <returns></returns>
         public virtual HeightElement.EncodeMode AddValue(int data) {
            int v = EvaluateData(LbsSum, data);
            ...
            EncodeMode = LbsSum > 0 ? HeightElement.EncodeMode.Length1 : HeightElement.EncodeMode.Length0;
         ...
         /// <param name="data"></param>
         /// <returns></returns>
         protected int EvaluateData(int sum, int data) {
            /*
               D < -2 – (ls + 3*k)/2   1 – ls – k	
               D < 0 – (ls + k)/2      2*(d + k) + 3	
               D < 2 – (ls – k)/2      2*d – 1	
            ...
            int v = 0;
            if (data < (-4 - sum - 3 * ElemCount) / 2) {
               v = -1 - sum - ElemCount;
            } else if (data < -(sum + ElemCount) / 2) {
               v = 2 * (data + ElemCount) + 3;
            } else if (data < (4 - sum + ElemCount) / 2) {
               v = 2 * data - 1;
            } else if (data < (8 - sum + 3 * ElemCount) / 2) {
               v = 2 * (data - ElemCount) - 5;
            } else {
               v = 1 - sum + ElemCount;
            }
            return v;
         ...
         public override string ToString() {
            return string.Format("EncodeMode={0}: LbsSum={1}, ElemCount={2}", EncodeMode, LbsSum, ElemCount);
         }

      }

      class LengthCodingTypePlateau0 : LengthCodingType {

         /// <summary>
         /// akt. Summe der Bewertungen
         /// </summary>
         public new int LbsSum {
            get {
               return base.LbsSum + 1;
            }
            protected set {
               base.LbsSum = value;
            }
         ...
         public LengthCodingTypePlateau0() : base() {
            //LbsSum = 1;
         }
         ...
         /// <param name="data"></param>
         /// <returns></returns>
         public override HeightElement.EncodeMode AddValue(int data) {
            int v = EvaluateData(base.LbsSum, data);
      ...
      /// </summary>
      LengthCodingTypePlateau0 lengthtype_ddiff4plateaufollower_zero;
      ...
      public TileEncoder(int maxheigth, int tilesize, IList<int> height) {
         ...
         lengthtype_ddiff4plateaufollower_zero = new LengthCodingTypePlateau0();
         ...
         if (ValidHeight(pos) >= 0) { // nur nichtnegative Höhen
            ...
            try {
               ...
               if (ddiff == 0) { // die Diagonale hat konstante Höhe (gilt auch für die 1. Spalte) -> immer Plateau (ev. auch mit Länge 0)
                  ...
                  bEnd = WritePlateau(GetPlateauLength(pos), pos, hu_ddiff4plateaufollower_zero, hu_ddiff4plateaufollower_notzero);
      ...
      /// <returns>true, wenn die Endpos. rechts-unten erreicht wurde</returns>
      bool WritePlateau(int length, Position pos, HeightUnit4PlateaufollowerDdiffZero hunit_followerddiffzero, HeightUnit4PlateaufollowerDdiffNotZero hunit_followerddiffnotzero) {
         bool bEnd;
         ...
         if (!bEnd)
            if (pos.X != 0 ||       // Das ist die Position für den Follower!
               ...
               if (follower_ddiff != 0) {
                  ...
                  lct = new LengthCodingType();
                  lct.AddValue(0); // --> Length0 erzwingen
