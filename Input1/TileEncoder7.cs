   ...
   public class TileEncoder {
      ...
      public class HeightElement {
         ...
         public class PlateauTable {
            ...
            /// Tabelle A oder B
            /// </summary>
            public bool IsTableA { get; private set; }
            ...
            void SetTable(bool bTableA) {
               if (bTableA)
                  table = new PlateauTableItem[] {
                           ...
                           new PlateauTableItem(32,6, '3'),
                           new PlateauTableItem(32,6, 'f'),
                           new PlateauTableItem(64,6, 'M'),
                           new PlateauTableItem(64,7, 'm'),
                        };
               else
                  table = new PlateauTableItem[] {
                           new PlateauTableItem(1,0, '1'),
            ...
            /// <summary>
            /// ändert die interne Tabelle (unwiderruflich) von A nach B
            /// </summary>
            public void Change2TableB() {
               if (IsTableA)
                  SetTable(false);
            }

            public override string ToString() {
               return string.Format("Idx={0}, IsTableA={1}", Idx, IsTableA);
            }
         ...
         void EncodePlateau(int length, int startcolumn, int startline, int linelength) {
            ...
            bool bHitBehindEnd = false;
            while ((PlateauTable4Tile.Unit(1) <= length) ||   // Unit kleiner als die restliche Länge ...
                   (startcolumn + length >= linelength)) {                 // ... oder noch nicht in der Endzeile
               ...
               if (unit == 16 &&                         // Unit 16 ...
                   PlateauTable4Tile.IsTableA &&         // ... und noch Tabelle A ...
                   PlateauTable4Tile.Unit(-1) == 8 &&    // ... und vorher Unit 8 ...
                   Bits.Count < 5) {                     // ... und weniger als 5x verwendet (d.h. keine 4x Unit 8)
                  PlateauTable4Tile.Change2TableB();
                  PlateauTable4Tile.IncrementIdx();
               }

               if (startcolumn + unit >= linelength) { // Zeilenwechsel
                  startline++;
                  if (startcolumn + unit > linelength) // Wenn NICHT genau das Zeilenende getroffen wird (Überlänge), wird der Index zum Schluß verkleinert.
                     bHitBehindEnd = true;
                  length -= linelength - startcolumn;
                  startcolumn = 0;

                  if (bHitBehindEnd &&
                      PlateauTable4Tile.Unit(1) < length) {   // Wenn es das letztes 1-Bit und ...
                     if (PlateauTable4Tile.Bits() >              // ... a, b, c, ... ist, erfolgt sofort das Dekr.
                         PlateauTable4Tile.Bits(-1)) {
      ...
      /// <summary>
      /// <see cref="MaxHeigth"/> ist Größe des Wertebereiches oder die max. zulässige Höhe der Kachel
      /// </summary>
      public bool MaxHeigthIsRange { get; protected set; }
      ...
      /// <param name="maxheigthisrange">Größe des Wertebereiches oder die max. zulässige Höhe der Kachel</param>
      ...
      public TileEncoder(int maxheigth, bool maxheigthisrange, int tilesize, IList<int> height) {
         MaxHeigth = maxheigth;
         MaxHeigthIsRange = maxheigthisrange;
         TileSize = tilesize;
         StdHeigth = 0;
      ...
      public override string ToString() {
         return string.Format("MaxHeigth={0}, MaxHeigthIsRange={1}, TileSize={2}, BaseHeigthUnit={3}, HeigthUnit={4}, ActualMode={5}, ActualHeigth={6}",
                              MaxHeigth,
                              MaxHeigthIsRange,
                              TileSize,
                              InitialHeigthUnit,
